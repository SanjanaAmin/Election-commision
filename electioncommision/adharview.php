      ...
      <li class="nav-item dropdown">
        ...
        <table border="1" width=100% bgcolor='white'>
           ...
           <?php
            ...
            $sql = "SELECT * FROM adhar";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                while($row = $result-> fetch_assoc()) {
                    echo "<tr><td>".$row["A_Fname"] . "</td><td>" .$row["A_Mname"] . "</td><td>" .$row["A_Lname"] . "</td><td>" .$row["A_id"] . "</td><td>" .$row["A_gender"] . "</td><td>" .$row["A_dob"] . "</td><td>" .$row["A_address"] . "</td><td>" .$row["A_contact"] . "</td></tr>";
